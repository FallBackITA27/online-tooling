...
async function refresh(rksysByteArray) {
    ...
    */
    ...
    for (let i = 0; i < 6; i++) {
		...
		let charElement = document.getElementById("char"+emId);
		let conElement = document.getElementById("con"+emId);
		let vehElement = document.getElementById("veh"+emId);
        if(ms===0&&seconds===0&&minutes===0) {
            ...
            charElement.innerHTML = ""; conElement.innerHTML = ""; vehElement.innerHTML = "";
            continue;
        }
        ...
        charElement.innerHTML = chars[parseInt(trackData[0x50].toString(2).padStart(8,"0").substring(1),2)];
        conElement.innerHTML = cons[parseInt(trackData[0x51].toString(2).padStart(8,"0").substring(0,3),2)];
        vehElement.innerHTML = vehs[parseInt(trackData[0x4F].toString(2).padStart(8,"0").substring(0,6),2)];
...
rksysInput.addEventListener("change", function(){
    ...
    reader.onloadend = function (e) {
        if (e.target.readyState == FileReader.DONE) {
            let arrayBuffer = e.target.result, array = new Uint8Array(arrayBuffer);
